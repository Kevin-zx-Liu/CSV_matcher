		...
		if all(col in full_df.columns for col in required_cols):
			...
			if not valid_df.empty:
				...
				try:
					...
					trend_suffix = f"{start_date}_to_{end_date}"
				except:
					trend_suffix = "history"

				chart1["usermeta"] = {
					"embedOptions": {
						"downloadFileName": f"Trend_{trend_suffix}"
					}
				}
				...
				if 'Reason' in valid_df.columns:
					...
					if not missing_df.empty:
						...
						chart2["usermeta"] = {
							"embedOptions": {
								"downloadFileName": f"MissingReasons_trend_{trend_suffix}"
							}
						}
